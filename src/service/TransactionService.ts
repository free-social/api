...
export class TransactionService {
  ...
  static async updateOne(
    ...
    if (invalidFields.length > 0)
      throw new Error(`Invalid update: ${invalidFields.join(", ")}`);
    ...
    try {
      ...
      if (data.amount !== undefined && data.amount !== oldTransaction.amount) {
        ...
        if (!wallet || wallet.balance < 0) {
          throw new Error(
            "Insufficient funds to update this transaction amount",
          );
        }
      }
  ...
  static async getDailyTransactions(userId: string, date?: Date) {
    try {
      // 1. Set target date in Phnom Penh timezone
      const targetDate = date
        ? moment(date).tz("Asia/Phnom_Penh")
        : moment().tz("Asia/Phnom_Penh");

      // 2. Start and end of day in Phnom Penh, then convert to UTC for MongoDB query
      const startOfDayUTC = targetDate.clone().startOf("day").utc().toDate();
      const endOfDayUTC = targetDate.clone().endOf("day").utc().toDate();

      // 3. Query transactions in UTC range
      ...
      return {
        totalSpent,
        date: targetDate.format("YYYY-MM-DD"), // Keep readable Phnom Penh date
  ...
  static async getMonthlyTransactions(
    ...
    try {
      ...
      const targetMonth =
        month && month >= 1 && month <= 12 ? month : now.month() + 1; // moment month is 0-indexed
      const targetYear = year || now.year();

      // 2. Start of month in Cambodia timezone
      ...
      // 3. Convert start and end of month to UTC for MongoDB query
      const startOfMonthUTC = startOfMonth.clone().utc().toDate();
      const endOfMonthUTC = startOfMonth.clone().endOf("month").utc().toDate();

      // 4. Query transactions in UTC range
      const transactions = await Transaction.find({
        user: userId,
      ...
      return {
        totalSpent,
        month: startOfMonth.format("MMMM"), // Month name in Cambodia timezone
        year: targetYear,
        count: transactions.length,
        transactions,
      };
